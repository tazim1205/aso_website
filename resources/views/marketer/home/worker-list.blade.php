...
@push('title') {{ __('Worker Under Me') }} @endpush
@push('head')
                    ...
                    <div class="col-12">
                        <table class="table table-bordered" id="datatable">
                            <thead>
                                ...
                                <tr class="" style="background: linear-gradient(45deg, #ffdf40, #ff8359)!important;">
                                    <th scope="col" class="text-center border-0">{{__('ওয়ার্কারের ইউসারনেম')}}</th>
                                    <th scope="col" class="text-center border-0">{{__('সাইন আপ তারিখ')}}</th>
                                    <th scope="col" class="text-center border-0">{{ get_static_option('job_complete_amount') }} {{__('টাকার জব কমপ্লিট')}}</th>
        ...
        $(document).ready( function () {
            function showWorkerList(year, month){
                var dataTable = $('#datatable').DataTable({
                    ...
                    // scrollX: true,
                    "order": [[ 0, "desc" ]],
                    ajax: "{{  url('/get/worker/under/me/') }}/"+year+"/"+month,
                    columns: [
                        {data: 'user_name', name: 'user_name',orderable:false},
                        {data: 'date', name: 'date'},
                        {data: 'jobcomplete', name: 'jobcomplete',sClass:'text-center'},
