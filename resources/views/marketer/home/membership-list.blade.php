...
@push('title') {{ __('Member Under Me') }} @endpush
@push('head')
                            ...
                            <thead>
                                <tr class="" style="background: linear-gradient(45deg, #ffdf40, #ff8359)!important;">
                                    <th colspan="3" class="text-center">
                                        {{__('পেইজ মেম্বারশীপ কমিশন বিস্তারিত')}}
                                    </th>
                                    <th colspan="3" class="text-right">
                                        <div class="row">
                                            <div class="col-6 col-lg-6 form-group">
                                                <select class=" bg-warning" id="monthForMembershipList" name="monthForMembershipList">
                                                    @for($i = 1; $i <= 12; $i++)
                                            ...
                                            <div class="col-6 col-lg-6 form-group">
                                                <select class=" bg-warning" id="yearForMembershipList" name="yearForMembershipList">
                                                    @for($i = 2021; $i <= 2050; $i++)
                                ...
                                <tr class="" style="background: linear-gradient(45deg, #ffdf40, #ff8359)!important;">
                                    <th scope="col" class="text-center border-0">{{__('পেইজ নাম')}}</th>
                                    <th scope="col" class="text-center border-0">{{__('ওয়ার্কার ইউজারনেম')}}</th>
                                    <th scope="col" class="text-center border-0">{{__('প্যাকেজ ক্রয়ের তারিখ')}}</th>
                                    <th scope="col" class="text-center border-0">{{__('ক্রয়কৃত প্যাকেজ')}}</th>
                                    <th scope="col" class="text-center border-0">{{__('প্রাপ্ত আয়')}}</th>
                                    <th scope="col" class="text-center border-0">{{__('বিস্তারিত')}}</th>
                                </tr>
                            </thead>
                            <tbody id="bodyOfMembershipListTable">
...
@push('foot')
    ...
    <script>
        $(document).ready( function () {
            function showMembershipList(year, month){
                var dataTable = $('#datatable').DataTable({
                    ...
                    // scrollX: true,
                    "order": [[ 0, "desc" ]],
                    ajax: "{{  url('/get/member/under/me/') }}/"+year+"/"+month,
                    columns: [
                        {data: 'pages', name: 'pages',orderable:false, serachable:false},
                        {data: 'user_name', name: 'user_name',orderable:false},
                        {data: 'date', name: 'date'},
                        {data: 'buy', name: 'buy',sClass:'text-center'},
                        {data: 'balance', name: 'balance',sClass:'text-center'},
                        {data: 'Actions', name: 'Actions',orderable:false,serachable:false,sClass:'text-center'},
                    ]
                });
            }
            showMembershipList(0,0);

            $('#yearForMembershipList').on('change', function(e){
                e.preventDefault();

                var year = $(this).val();
                var month = $('#monthForMembershipList').val();
                $('#datatable').DataTable().destroy();
                showMembershipList(year,month);
            });

            $('#monthForMembershipList').on('change', function(e){
                e.preventDefault()

                var year = $('#yearForMembershipList').val();
                var month = $(this).val();
                $('#datatable').DataTable().destroy();
                showMembershipList(year,month);
            });
