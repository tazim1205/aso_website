                            ...
                            <div class="modal-body">
                                <form action="#" id="add-new-from">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <textarea id="description" rows="5" class="form-control" name="comment" id="detail" placeholder="Enter Description"></textarea>
                                    </div>
                            ...
                            <form class="form">
                                <div class="form-body">
                                    <h4 class="form-section">
                                        <i class="ft-flag"></i>Notice</h4>
                                    <input type="hidden" class="hidden-id" value="{{ $notice->id }}">
                                    <div class="form-group">
                                        <label for="title">Title</label>
                                        <input type="text" id="title" class="form-control" placeholder="Enter Title" value="{{ $notice->title }}" name="title">
                                    </div>
                                    <div class="form-group">
                                        <label for="description">Description</label>
                                        <textarea id="description" rows="5" class="form-control" name="description" placeholder="Enter Description">
                                            {!! $notice->detail !!}
                                          </textarea>
                                    </div>
                                </div>
                                ...
                                <div class="form-actions">
                                    ...
                                    <button type="button" class="btn btn-primary float-right mb-1 mx-2 delete-submit-button" data-id="{{ $notice->id }}">
                                        Delete
                                    </button>
                                    <button
                                        type="button"
                                        class="btn btn-primary float-right mb-1 update-submit-button"
                                        data-id="{{ $notice->id }}"
                                        data-title="{{ $notice->title }}"
                                        data-description="{{ $notice->detail }}"
                                    >
                                        Update
                                    </button>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#add-submit-button').click(function(){
                ...
                $.ajax({
                    method: 'POST',
                    url: '{{ route('marketing_panel.notice.store') }}',
            ...
            $('.update-submit-button').click(function(){
                ...
                $.ajax({
                    method: 'POST',
                    url: "{{ route('marketing_panel.notice.update') }}",
                    headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                    data: formData,
                    processData: false,
                    contentType: false,
            ...
            $('.delete-submit-button').click(function(){
                ...
                $.ajax({
                    method: 'POST',
                    url: "{{ route('marketing_panel.notice.delete') }}",
                    headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                    data: formData,
                    processData: false,
                    contentType: false,
                    success: function (data) {
                        ...
                        Swal.fire({
                            position: 'top-end',
                            icon: 'danger',
                            title: 'Successfully deleted '+data.title,
                            showConfirmButton: false,
                            timer: 1500
                        })
                        setTimeout(function() {
